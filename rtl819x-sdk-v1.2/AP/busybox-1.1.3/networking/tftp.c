	...
	const char *remotefile, int localfd, const unsigned short port, int tftp_bufsize)
{
    ...
    char *rbuf = xmalloc(tftp_bufsize + 4);
	...
	while (1) {
		...
		*((unsigned short *) cp) = htons(opcode);
		...
		if ((cmd_get && (opcode == TFTP_RRQ)) ||
			...
			if (too_long || ((&xbuf[tftp_bufsize - 1] - cp) < 6)) {
		...
		do {
			...
			case 1:
                ...
                bb_error_msg("fall-through\n");
			case 0:
				bb_error_msg("timeout, block_nr=%d", block_nr);

				timeout--;
				if (timeout == 0) {
					len = -1;
					bb_error_msg("last timeout, block_nr=%d", block_nr);
				}
				break;
		...
		if (want_option_ack) {
			 ...
			 want_option_ack = 0;
				 ...
				 res = tftp_option_get(&rbuf[2], len-2,
						       "blksize");
		...
		if (cmd_get && (opcode == TFTP_DATA)) {
			...
			if (tmp == block_nr) {
				...
				len = bb_full_write(localfd, &rbuf[4], len - 4);
