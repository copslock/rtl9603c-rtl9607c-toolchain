 ...
 *
 * $Id: webs.c,v 1.22 2010/04/26 07:01:08 keith_huang Exp $
 */
...
#if defined(CONFIG_RTL8196B_GW_8M)
do{
                    preAlloc = brealloc(B_L, wp->postData, (wp->lenPostData + nbytes + 1) * sizeof(char_t));
			if(preAlloc==NULL)
			{
				sleep(1);
				//fprintf(stderr,"\r\n brealloc again!!,__[%s-%u]",__FILE__,__LINE__);
			}
		}while(preAlloc == NULL);
			
                    wp->postData = preAlloc;
#else
...
#if defined(CONFIG_RTL8196B_GW_8M)
while(NULL == brealloc(B_L, wp->postData, lenUploadedFile))
			{
//fprintf(stderr,"\r\n brealloc FAIL!!__[%s-%u]",__FILE__,__LINE__);			
				}
#else
