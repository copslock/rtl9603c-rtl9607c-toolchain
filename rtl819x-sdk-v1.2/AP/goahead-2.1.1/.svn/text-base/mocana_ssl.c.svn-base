...
static unsigned char*    pCertificate    = NULL;
static unsigned char*    pRsaKeyBlob     = NULL;
...
static int  mocana_SSL_computeHostKeys(unsigned char** ppCertificate, unsigned int *pCertLength,
                                       unsigned char** ppRsaKeyBlob,  unsigned int *pKeyBlobLength);
static int  mocana_SSL_releaseHostKeys(unsigned char **ppCertificate, unsigned char **ppRsaKeyBlob);
...
int websSSLOpen(char_t* certPath, int sslPort)
{
    ...
    if (0 > SSL_init(MAX_SSL_CONNECTIONS_ALLOWED, 0)) 
      return -1;

    if (0 > SSL_initServer(pCertificate, certLength, pRsaKeyBlob, rsaKeyBlobLength))
        return -1;

    /* lumentis */
    SSL_sslSettings()->sslListenPort = sslPort;
    ...
    /* lumentis */
    if (sslPort != 0) {
      sslListenSock = socketOpenConnection(NULL, sslPort, websSSLAccept, SOCKET_BLOCK);
    }
    else {
...
int websSSLRead(websSSL_t *wsp, char_t *buf, int len)
{
    ...
    if ((rc = SSL_recv(wsp->mocanaConnectionInstance, buf, len, &numBytesReceived, TIMEOUT_SSL_RECV)) < 0)
    {
        if (0 > rc)
            rc = -1;
 ...
 */
    ...
    {
      trace(8, T("websSSLReadEvent: SSL_acceptConnection failed. <%d>\n"), connectionInstance);
    ...
    if (0 > (status = SSL_negotiateConnection(connectionInstance))) {
      trace(8, T("websSSLReadEvent: SSL_negotiateConnection failed. <%d>\n"), status);
#ifdef __ENABLE_ALL_DEBUGGING__
        printf("websSSLReadEvent: SSL_negotiateConnection failed. %d\n", status);
#endif
...
int    websSSLGets(websSSL_t *wsp, char_t **buf)
{
    ...
    *buf = NULL;
    ...
    while (1) {
        ...
        if ((rc = SSL_recv(mci, &c, 1, &numBytesReceived, TIMEOUT_SSL_RECV)) < 0)
        {
	  if (0 > rc) {
	      trace(8, T("websSSLGets: SSL_recv() failed <%d>\n"), rc);
...
static int
mocana_SSL_testHostKeys(unsigned char** ppCertificate, unsigned int *pCertLength,
                         unsigned char** ppRsaKeyBlob,  unsigned int *pKeyBlobLength)
{
    int             status;
...
static int
mocana_SSL_releaseHostKeys(unsigned char **ppCertificate, unsigned char **ppRsaKeyBlob)
{
...
static int
mocana_SSL_computeHostKeys(unsigned char** ppCertificate, unsigned int *pCertLength,
                            unsigned char** ppRsaKeyBlob,  unsigned int *pKeyBlobLength)
{
    int   status;
    ...
    *ppRsaKeyBlob  = NULL;
    ...
    {
      trace(8, T("mocana_SSL_computeHostKeys: host keys do not exist, computing new key pair.\n"));
#ifdef __ENABLE_ALL_DEBUGGING__
	...
	trace(8, T("mocana_SSL_computeHostKeys: host key computation completed.\n"));
#ifdef __ENABLE_ALL_DEBUGGING__
