...
#define SSL_CERTFILE "cert.pem"
#define SSL_PRIVFILE "key.pem"
...
int websSSLOpen(char_t* certPath, int sslPort)
{
    ...
    privFile = (char*) malloc(strlen(certPath) + strlen(SSL_PRIVFILE) + 2);
    sprintf(privFile, "%s/%s", certPath, SSL_PRIVFILE);
    certFile = (char*) malloc(strlen(certPath) + strlen(SSL_CERTFILE) + 2);
    sprintf(certFile, "%s/%s", certPath, SSL_CERTFILE);
...
static int websSSLReadEvent (webs_t wp)
{
    ...
    if (0 > sslAccept(&sslConn, sock, sslKeys)) {
      /* tbd debug */
