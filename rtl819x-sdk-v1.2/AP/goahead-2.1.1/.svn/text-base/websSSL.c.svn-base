 ...
 *
 * $Id: websSSL.c,v 1.1.1.1 2007/08/06 10:04:43 root Exp $
 */

#ifdef WEBS_SSL_SUPPORT
#if defined(RSA_SSL) || defined(OPENSSL) 
...
#define DEFAULT_CERT_FILE	"servercert.pem"
#define DEFAULT_KEY_FILE	"serverkey.pem"
#define DEFAULT_CAKEY_FILE	"cakey.pem"
#define DEFAULT_CACERT_FILE	"cacert.pem"
...
//#define DEFAULT_CA_FILE		"certs/cert.pem"
#define DEFAULT_CA_PATH		"/etc"
#define SSL_PORT			443
#endif
...
{
	char		certFile[255], CAfile[255], keyFile[255];
	char		*CApath;
	SSL_METHOD	*meth;
 ...
 */
	/* lum: user option for path */
	if (certPath == NULL) { 
	CApath = DEFAULT_CA_PATH;
	  (void) strncpy(CAfile, DEFAULT_CA_PATH DEFAULT_CACERT_FILE, sizeof(CAfile));
	}
	else {
	  CApath = certPath;
	  (void) strncpy(CAfile, certPath, sizeof(CAfile));
	  (void) strncpy(CAfile + strlen(certPath), "/" DEFAULT_CACERT_FILE, sizeof(CAfile) - strlen(certPath) - 1);
	}
 ...
 */
	/* lum: user option for path */
	if (certPath == NULL) { 
	  (void) strncpy(certFile, DEFAULT_CERT_PATH DEFAULT_CERT_FILE, sizeof(certFile)-1);
	  (void) strncpy(keyFile, DEFAULT_CERT_PATH DEFAULT_KEY_FILE, sizeof(keyFile)-1);
	}
	else {
	  (void) strncpy(certFile, certPath, sizeof(certFile)-1);
	  (void) strncpy(certFile + strlen(certPath), "/" DEFAULT_CERT_FILE, sizeof(certFile)-strlen(certPath)-1);
	  (void) strncpy(keyFile, certPath, sizeof(keyFile)-1);
	  (void) strncpy(keyFile + strlen(certPath), "/" DEFAULT_KEY_FILE, sizeof(keyFile)-strlen(certPath)-1);
	}
 ...
 */
	/* Lumentis: */
	if (sslPort) {
	  sslListenSock = socketOpenConnection(NULL, sslPort, 
