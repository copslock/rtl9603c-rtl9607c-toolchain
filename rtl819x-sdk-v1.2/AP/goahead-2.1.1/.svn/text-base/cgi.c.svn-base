 ...
 * $Id: cgi.c,v 1.1.1.1 2007/08/06 10:04:43 root Exp $
...
//		(sbuf.st_size > cgip->fplacemark)) {
	...
	if ((cp = gstrrchr(cgip->cgiPath, '/')) != NULL)
		*cp = '\0';

	sprintf(tmpBuf, "%s/%s",  cgip->cgiPath, cgip->stdOut);

	if ((gstat(tmpBuf, &sbuf) == 0) &&
		(sbuf.st_size > cgip->fplacemark)) {
...
//		fdout = gopen(cgip->stdOut, O_RDONLY | O_BINARY, 0444 );
		fdout = gopen(tmpBuf, O_RDONLY | O_BINARY, 0444 );
