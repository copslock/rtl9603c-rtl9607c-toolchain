...
static int createKeyBlock(ssl_t *ssl, char *clientRandom, char *serverRandom, 
						  char *masterSecret, int secretLen);
...
/*
	master_secret =
		...
		MD5(pre_master_secret + SHA('CCC' + pre_master_secret +
			ClientHello.random + ServerHello.random));
*/
	tmp = tmp2 = sslMalloc(SSL_HS_PREMASTER_SIZE);
	for (i = 0; i < 3; i++) {
...
/*
 *	This is as good a place as any to set up the encrypt and decrypt contexts
 */
	if (ssl->cipher->init(&(ssl->sec)) < 0) {
		matrixStrDebugMsg("Unable to initialize cipher suite\n", NULL);
		return -1;
	}
...
*/
	...
	ssl->sec.keyBlock = sslMalloc(SSL_MD5_HASH_SIZE * keyIter);
...
*/
	if (ssl->flags & SSL_FLAGS_SERVER) {
		ssl->sec.readMAC = ssl->sec.keyBlock;
		ssl->sec.writeMAC = ssl->sec.readMAC + ssl->cipher->macSize;
		ssl->sec.readKey = ssl->sec.writeMAC + ssl->cipher->macSize;
		ssl->sec.writeKey = ssl->sec.readKey + ssl->cipher->keySize;
		ssl->sec.readIV = ssl->sec.writeKey + ssl->cipher->keySize;
		ssl->sec.writeIV = ssl->sec.readIV + ssl->cipher->ivSize;
	} else {
		ssl->sec.writeMAC = ssl->sec.keyBlock;
		ssl->sec.readMAC = ssl->sec.writeMAC + ssl->cipher->macSize;
		ssl->sec.writeKey = ssl->sec.readMAC + ssl->cipher->macSize;
		ssl->sec.readKey = ssl->sec.writeKey + ssl->cipher->keySize;
		ssl->sec.writeIV = ssl->sec.readKey + ssl->cipher->keySize;
		ssl->sec.readIV = ssl->sec.writeIV + ssl->cipher->ivSize;
	}
...
/***************************************************************************************/
/*
	Combine the running hash of the handshake mesages with some constants and mix
	them up a bit more.  Output the result to the given buffer.  This data will
	be part of the Finished handshake message.
*/
...
*/
	matrixMd5Update(md5, (sender & SSL_FLAGS_SERVER) ? SENDER_SERVER : SENDER_CLIENT, 4);
...
/***************************************************************************************/
/*
	SSLv3 uses a method similar to HMAC to generate the SHA1 message MAC.  For SHA1,
	40 bytes of the pad are used.
...
/***************************************************************************************/
/*
	SSLv3 uses a method similar to HMAC to generate the MD5 message MAC.  For MD5,
	48 bytes of the pad are used.
