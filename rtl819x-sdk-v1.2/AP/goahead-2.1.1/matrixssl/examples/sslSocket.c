...
*/
	...
	conn->fd = fd;
	if (matrixSslNewSession(&conn->ssl, keys, NULL, SSL_FLAGS_SERVER) < 0) {
...
*/
	if (rc == 0) {
		...
		fprintf(stderr, "sslRead error in sslConnect\n");
		goto error;
	}
	*cpp = conn;
	return 0;
...
*/
		if (cp->inbuf.buf == (unsigned char*)buf) {
			cp->inbuf.buf = NULL;
			return (int)(cp->inbuf.end - cp->inbuf.start);
		}
/*
...
*/
	case SSL_PARTIAL:
		if (cp->insock.start == cp->insock.buf && cp->insock.end == 
			...
			if (cp->insock.size > SSL_MAX_RECORD_SIZE) {
				goto readError;
			}
			cp->insock.size *= 2;
			cp->insock.start = cp->insock.buf = 
...
retryEncode:
		...
		case SSL_ERROR:
			return -1;
		case SSL_FULL:
			if (cp->outsock.size > SSL_MAX_RECORD_SIZE) {
				return -1;
			}
			cp->outsock.size *= 2;
			cp->outsock.buf = 
