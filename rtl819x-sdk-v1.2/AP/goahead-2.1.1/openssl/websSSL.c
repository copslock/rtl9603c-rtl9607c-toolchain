...
#define SSL_PORT			443
#else
#define DEFAULT_CERT_FILE	"/etc/cert.pem"
#define DEFAULT_KEY_FILE	"/etc/key.pem"
...
int websSSLSetCertStuff(SSL_CTX *ctx, char *certFile, char *keyFile)
{
	...
	if (certFile != NULL) {
		if ((val = SSL_CTX_use_certificate_file(ctx, certFile, 
			SSL_FILETYPE_PEM)) <= 0) {
			perror("ssl_ctx:");
			trace(2, T("SSL: Unable to set certificate file <%s> %d\n"),
				certFile, val); 
			return -1;
		}
