 ...
 *
 *      $Id: mgbin.c,v 1.8 2009/11/20 05:17:41 bradhuang Exp $
 *
 */
#include <string.h>
#include <stdio.h>
...
static TYPE_T checkInputFile(char *filename, int *pWith_header)
{
	...
	*pWith_header = 1;
	...
	else if ( !memcmp(signature, "h", 1) || !memcmp(signature, "H", 1))
		return CONFIG;
	else if ( !memcmp(signature, "6G", 2) || !memcmp(signature, "6A", 2) || !memcmp(signature, "6g", 2) || !memcmp(signature, "6a", 2)) {
		no_hw_config = 1;
		return CONFIG;	
	}
