 ...
 *
 *      $Id: fmfwall.c,v 1.39.2.1 2010/12/16 05:32:20 bruce Exp $
 *
 */
...
void formFilter(webs_t wp, char_t *path, char_t *query)
{
	...
	if (strAddIp[0] || strDelIp[0] || strDelAllIp[0]) {
		...
		num_id = MIB_URLFILTER_NUM;
		get_id = MIB_URLFILTER;
...
int vlanList(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	cmpResult= strncmp(argv[0], "webVlanList_", strlen("webVlanList_"));
	...
	if( index <= MAX_IFACE_VLAN_CONFIG && index != 0) /* ignore item 0 */
	{
		...
		*((char *)&entry) = (char)index;
		if ( !apmib_get(MIB_VLANCONFIG, (void *)&entry))
		{
		...
		switch(index)
		{
			...
			case 4:
				...
				break;
			case 5:
				sprintf(IfaceName,"%s","Wireless Primary AP");
				if(opmode == WISP_MODE)
				{
					sprintf(wanLan,"%s","WAN");
#if defined(CONFIG_RTK_VLAN_ROUTETYPE)
					forwarding = VLAN_FORWARD_NAT;
#endif
				}
				else
				{
...
#if defined(CONFIG_RTK_VLAN_ROUTETYPE)
		websWrite(wp, T("%d|%s|%d|%d|%d|%d|%d|%d|%s|%d"), entry.enabled,IfaceName,entry.tagged,0,entry.priority,entry.cfi,0,entry.vlanId,wanLan,forwarding);
#else
	...
	{
#if defined(CONFIG_RTK_VLAN_ROUTETYPE)
		websWrite(wp, T("0|none|0|0|0|0|0|0|LAN|0") );
#else
		websWrite(wp, T("0|none|0|0|0|0|0|0|LAN") );
#endif
	}
	return 0;
}
...
int urlFilterList(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	for (i=1; i<=entryNum; i++) {
		*((char *)&entry) = (char)i;
		if ( !apmib_get(MIB_URLFILTER, (void *)&entry))
			return -1;
...
int qosList(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	{
		*((char *)&entry) = (char)index;
		if ( !apmib_get(MIB_QOS_RULE, (void *)&entry))
		{
			goto ret_empty;
		}
...
/*        *((char *)&entry_existed) = (char)index;
        if ( !apmib_get(MIB_QOS_RULE, (void *)&entry_existed)) {
		strcpy(tmpBuf, T("Get table entry error!"));
		goto setErr_qos;
        }
