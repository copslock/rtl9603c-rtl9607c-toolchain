 ...
 *
 *      $Id: fmwlan.c,v 1.86.2.6 2011/01/21 08:43:54 keith_huang Exp $
 *
 */
...
void formSchedule(webs_t wp, char_t *path, char_t *query)
{
	...
	if ( !apmib_set(MIB_SCHEDULE_DELALL, (void *)&entry)) {
	...
	apmib_set(MIB_SCHEDULE_ENABLED,(void *)&isEnabled);
	...
	if ( !apmib_get(MIB_SCHEDULE_NUM, (void *)&entryNum)) 
	{
			strcpy(tmpBuf, T("\"Get entry number error!\""));
			goto setErr_schedule;
	}
	if ( !apmib_set(MIB_SCHEDULE_ADD,(void *)&entry)) {
...
int getScheduleInfo(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	apmib_get(MIB_SCHEDULE_ENABLED,(void *)&isEnabled);
	if(isEnabled==0){
		...
		for (i=1; i<=entryNum; i++) {
				*((char *)&entry) = (char)i;
				if ( !apmib_get(MIB_SCHEDULE, (void *)&entry)){
...
int wlanHandler(webs_t wp, char *tmpBuf, int *mode, int wlan_id)
{
	...
	if ( strVal[0] ) 
	{
		...
		if(wlan_onoff_tkip == 0) //Wifi request
		{
			...
			if(*mode != CLIENT_MODE && (band_no == 7 || band_no == 9 || band_no == 10)) //7:n; 9:gn; 10:bgn
			{
...
{	int id;
	...
	if ((strVal==NULL || strVal[0]==0) &&  // not called from wizard	
		...
		if (val)
			...
			disable = 1;		
		apmib_set(MIB_WLAN_DISABLED, (void *)&disable);
...
int meshWpaHandler(webs_t wp, char *tmpBuf, int wlan_id)
{
	...
	if(encrypt == ENCRYPT_WPA2)
	{
		...
		if ( getPSK ) {
			...
			apmib_get(MIB_MESH_WPA_PSK_FORMAT, (void *)&oldFormat);
			...
			if ( apmib_set(MIB_MESH_WPA_PSK_FORMAT, (void *)&intVal) == 0) {
				strcpy(tmpBuf, T("Set MIB_MESH_WPA_PSK_FORMAT failed!"));
				goto setErr_mEncrypt;
			}
...
int surveyHandle(webs_t wp, char *tmpBuf, int wlan_id)
{
	char_t *tmpStr, *strVal, *strWpaKeyValue, *strChan;
...
//displayPostDate(wp->postData);	
	...
	sprintf(varName, "survey_channel%d", wlan_id);
	strChan = websGetVar(wp, varName, T(""));
	if ( strChan[0] ) {
		chan = strtol( strChan, (char **)NULL, 10);
		apmib_set( MIB_WLAN_CHAN_NUM, (void *)&chan);
		
		apmib_get(MIB_WLAN_CHANNEL_BONDING, (void *)&intVal);			
		if(intVal == 1) //40 MHz
		{			
			if( chan >= 1 && chan<= 4)
			{
				intVal = 1; //lower
				apmib_set(MIB_WLAN_CONTROL_SIDEBAND, (void *)&intVal);
			}
			else if( chan >= 8 && chan<= 11)
			{
				intVal = 0; //upper
	...
	sprintf(varName, "is_rpt_enabled%d", wlan_id);
	tmpStr = websGetVar(wp, varName, T(""));
	if(tmpStr[0])
		intVal = (atoi(tmpStr))?0:1;
	
	apmib_set(MIB_WLAN_DISABLED, (void *)&intVal);
			
	sprintf(varName, "survey_ssid%d", wlan_id);
	tmpStr = websGetVar(wp, varName, T(""));	
	...
	apmib_set(mibId, (void *)tmpStr);
		
	sprintf(varName, "survey_method%d", wlan_id);
	tmpStr = websGetVar(wp, varName, T(""));
	...
	if(tmpStr[0])
	{
		...
		{			
			char_t *strKeyLen, *strFormat, *wepKey;
			WEP_T wep;
			int i, keyLen;
			char key[30];
			...
			sprintf(varName, "survey_length%d", wlan_id);
			strKeyLen = websGetVar(wp, varName, T(""));
			if (!strKeyLen[0]) {
	 			strcpy(tmpBuf, T("survey Key length must exist!"));
				goto setErr_surveyHandle;
			}
			if (strKeyLen[0]!='1' && strKeyLen[0]!='2') {
	 			strcpy(tmpBuf, T("Invalid survey key length value!"));
				goto setErr_surveyHandle;
			}
			if (strKeyLen[0] == '1')
				wep = WEP64;
			...
			if ( apmib_set( MIB_WLAN_WEP, (void *)&wep) == 0) {
		  		strcpy(tmpBuf, T("Set survey WEP MIB error!"));
				goto setErr_surveyHandle;
			}
		
			sprintf(varName, "survey_format%d", wlan_id);
			strFormat = websGetVar(wp, varName, T(""));
			if (!strFormat[0]) {
		 		strcpy(tmpBuf, T("survey Key type must exist!"));
				goto setErr_surveyHandle;
			}
		
			if (strFormat[0]!='1' && strFormat[0]!='2') {
				strcpy(tmpBuf, T("Invalid survey key type value!"));
				goto setErr_surveyHandle;
			}
		
			i = strFormat[0] - '0' - 1;
			if ( apmib_set( MIB_WLAN_WEP_KEY_TYPE, (void *)&i) == 0) {
		  		strcpy(tmpBuf, T("Set survey WEP key type error!"));
				goto setErr_surveyHandle;
			}
			...
			sprintf(varName, "survey_key%d", wlan_id);
			wepKey = websGetVar(wp, varName, T(""));
			if  (wepKey[0]) 
			{
				if (strlen(wepKey) != keyLen) 
				{
					strcpy(tmpBuf, T("Invalid survey key length!"));
					goto setErr_surveyHandle;
				}
				if ( !isAllStar(wepKey) ) 
				{
					...
					{ 
						if ( !string_to_hex(wepKey, key, keyLen)) {
			   				strcpy(tmpBuf, T("Invalid survey wep-key value!"));
							goto setErr_surveyHandle;
						}
					}
					if (wep == WEP64){
					...
					if (!ret) {
			 			strcpy(tmpBuf, T("Set survey wep-key error!"));
						goto setErr_surveyHandle;
					}
				}
			}
		...
		{
			int intVal, getPSK, len;
			encrypt=val;		
			
			apmib_set( MIB_WLAN_ENCRYPT, (void *)&encrypt);
			
			// WPA authentication
			sprintf(varName, "survey_wpaAuth%d", wlan_id);
			strVal = websGetVar(wp, varName, T(""));
			if (strVal[0]) 
			{
				if ( !gstrcmp(strVal, T("psk"))) {
					intVal = WPA_AUTH_PSK;
					getPSK = 1;
				}
			
				if ( apmib_set(MIB_WLAN_WPA_AUTH, (void *)&intVal) == 0) {
					strcpy(tmpBuf, T("Set survey MIB_WLAN_AUTH_TYPE failed!"));
					goto setErr_surveyHandle;
				}
			}

			if ((encrypt == ENCRYPT_WPA) || (encrypt == ENCRYPT_WPA2_MIXED)) {
				sprintf(varName, "survey_ciphersuite%d", wlan_id);
				strVal = websGetVar(wp, varName, T(""));	 	
				if (strVal[0]) 
				{
					intVal = 0;				
					...
					if ( gstrstr(strVal, T("aes"))) 
						intVal |= WPA_CIPHER_AES;
					if (intVal == 0) {
						strcpy(tmpBuf, T("Invalid value of survey cipher suite!"));
						goto setErr_surveyHandle;
					}
				}
				else
				...
				if ( apmib_set(MIB_WLAN_WPA_CIPHER_SUITE, (void *)&intVal) == 0) {
					strcpy(tmpBuf, T("Set survey MIB_WLAN_WPA_CIPHER_SUITE failed!"));
					goto setErr_surveyHandle;
				}				
			}			
			...
			{
				sprintf(varName, "survey_wpa2ciphersuite%d", wlan_id);
				strVal = websGetVar(wp, varName, T(""));	 	
				...
				if (strVal[0]) {
					...
					if ( gstrstr(strVal, T("aes"))) 
						intVal |= WPA_CIPHER_AES;
					if (intVal == 0) {
						strcpy(tmpBuf, T("Invalid value of survey wpa2 cipher suite!"));
						goto setErr_surveyHandle;
					}
				}
				else
				...
				if ( apmib_set(MIB_WLAN_WPA2_CIPHER_SUITE, (void *)&intVal) == 0) {
					strcpy(tmpBuf, T("Set survey MIB_WLAN_WPA2_CIPHER_SUITE failed!"));
					goto setErr_surveyHandle;
				}
			}
			...
			if ( getPSK ) 
			{
				...
				sprintf(varName, "survey_pskValue%d", wlan_id);
				strWpaKeyValue = websGetVar(wp, varName, T(""));				
				
				if ( isAllStar(strWpaKeyValue) == 0 )
				{
					sprintf(varName, "survey_pskFormat%d", wlan_id);
	   			strVal = websGetVar(wp, varName, T(""));
					if (!strVal[0]) {
			 			strcpy(tmpBuf, T("Error! no survey psk format."));
						goto setErr_surveyHandle;
					}
					...
					if (intVal != 0 && intVal != 1) 
					{
			 			strcpy(tmpBuf, T("Error! invalid survey psk format."));
						goto setErr_surveyHandle;
					}
					else
					{
						if ( apmib_set(MIB_WLAN_WPA_PSK_FORMAT, (void *)&intVal) == 0) {
							strcpy(tmpBuf, T("Set survey MIB_WLAN_WPA_PSK_FORMAT failed!"));
							goto setErr_surveyHandle;
						}
					}
									
					len = strlen(strWpaKeyValue);						
															
					if (intVal==1) { // hex
						if (len!=MAX_PSK_LEN || !string_to_hex(strVal, tmpBuf, MAX_PSK_LEN)) 
						{
		 					strcpy(tmpBuf, T("Error! invalid survey psk value."));
							goto setErr_surveyHandle;
						}
					}
					else 
					...
					if ( !apmib_set(MIB_WLAN_WPA_PSK, (void *)strWpaKeyValue)) {
						strcpy(tmpBuf, T("Set survey MIB_WLAN_WPA_PSK error!"));
						goto setErr_surveyHandle;
					}
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		...
		if( apmib_get(MIB_WLAN_MAC_ADDR, (void *)mac)<0 )
	...
	strValue = websGetVar(wp, T("tx_stbc"), T(""));
		if (strValue[0]) {
			if (!strcmp(strValue, T("enable")))
				...
				val = 0;	
			apmib_set(MIB_WLAN_11N_STBC, (void *)&val);	
		}
		else
		{		
	...
	strValue = websGetVar(wp, T("coexist_"), T(""));
		if (strValue[0]) {
			if (!strcmp(strValue, T("enable")))
				...
				val = 0;	
			apmib_set(MIB_WLAN_11N_COEXIST, (void *)&val);	
		}
#ifdef WIFI_SIMPLE_CONFIG
...
void formWlanMultipleAP(webs_t wp, char_t *path, char_t *query)
{
	...
	for (idx=1; idx<=4; idx++) {
		...
		if ( !gstrcmp(strVal, T("ON")))
			...
			disabled = 1;	
		apmib_set(MIB_WLAN_DISABLED, (void *)&disabled);
		...
		val = 0;
		apmib_set(MIB_WLAN_BASIC_RATE, (void *)&val);		
		apmib_set(MIB_WLAN_SUPPORTED_RATE, (void *)&val);
				...
				{ 
					...
					for(i = 0; i < 10; i++)	// This block might be removed when the mesh peerlink precedure has been completed
					{
						if(!getWlMeshLink(WLAN_IF, pStatus->bssdb[idx].bdBssId, 6))
						{
							...
							if ( apmib_set(MIB_WLAN_CHAN_NUM, (void*)&channel) == 0)
							{ 
...
// ==== inserted by GANTOE for site survey 2008/12/26 ==== 
	...
	memset(showAllStr,0x00,sizeof(showAllStr));
	if(argc > 0)
	{
		sprintf(showAllStr, "%s",argv[0]);
	}
...
//#ifdef CONFIG_RTK_MESH
	...
	apmib_get( id, (void *)&isRptEnabled);

	if ( mode == CLIENT_MODE || (mode == AP_MODE && isRptEnabled == 1) || strcmp(showAllStr,"all") == 0)
#else //#if defined(CONFIG_RTL8196C_AP_ROOT)
	if ( mode == CLIENT_MODE)
...
////////////////////////////////////////////////////////////////////////////////
void apmib_reset_wlan_to_default(void)
{
	memcpy(&pMib->wlan[0],  &pMibDef->wlan[0], sizeof(CONFIG_WLAN_SETTING_T));	
}

void formWsc(webs_t wp, char_t *path, char_t *query)
{
	char_t *strVal, *submitUrl, *strResetUnCfg;
	int intVal;
	char tmpbuf[200];
	int mode;
...
//	printf("MAC %s \n",strVal);
	...
	{
		mPolicy=strVal[0]-'0';
		if(!apmib_set(MIB_WLAN_WAPI_MCAST_REKEYTYPE,(void *)&mPolicy))
		{
...
//		printf("REKEY_M_PACKET %s \n",strVal);
	...
	{
		policy=strVal[0]-'0';
		if(!apmib_set(MIB_WLAN_WAPI_UCAST_REKETTYPE,(void *)&policy))
		{
...
//			printf("tmpBuf=%s\n",tmpBuf);//Added for test
	...
	if(5 == operation)
	{
		...
		{
			val=strVal[0]-'0';
			if(!apmib_set(MIB_WLAN_WAPI_SEARCH_CERTINDEX,(void*)&val))
			{
...
//		printf("SELECT1 %s\n",strVal);
		...
		if(!apmib_set(MIB_WLAN_WAPI_SEARCH_CERTINFO,(void*)strVal))
		{
...
//		printf("CERT_INFO %s\n",strVal);
	...
	if(6 == operation)
	{
	  ...
	  { 
	   ...
	   apmib_set( MIB_TIME_YEAR, (void *)&cur_time);
	   cur_time = tm_time->tm_mon;
	   apmib_set( MIB_TIME_MONTH, (void *)&cur_time);
	   cur_time = tm_time->tm_mday;
	   apmib_set( MIB_TIME_DAY, (void *)&cur_time);
	   cur_time = tm_time->tm_hour;
	   apmib_set( MIB_TIME_HOUR, (void *)&cur_time);
	   cur_time = tm_time->tm_min;
	   apmib_set( MIB_TIME_MIN, (void *)&cur_time);
	   cur_time = tm_time->tm_sec;
	   apmib_set( MIB_TIME_SEC, (void *)&cur_time);
