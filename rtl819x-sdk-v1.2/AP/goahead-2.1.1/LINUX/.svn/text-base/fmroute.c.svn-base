	...
	while (fgets(buff, sizeof(buff), fp) != NULL) {
		if (nl) {
			...
			if (flgs & RTF_UP) {
				...
				if ( dhcp == PPPOE || dhcp == PPTP || dhcp == L2TP){
					if(checkentry->_interface_==1){
						if(!memcmp(checkentry->dstAddr, &entry.dstAddr, 4) && entry._interface_ == checkentry->_interface_ ){
							isFound=1;		
							break;
						}
					}else{
						if(!memcmp(checkentry->dstAddr, &entry.dstAddr, 4) && !memcmp(checkentry->gateway, &entry.gateway, 4) && entry._interface_ == checkentry->_interface_ ){
							isFound=1;		
							break;
						}
					}
		...
		*((char *)&entry) = (char)i;
		...
		nBytesSent += websWrite(wp, T("<tr>"
			"<td align=center width=\"23%%\" bgcolor=\"#C0C0C0\"><font size=\"2\">%s</td>\n"
				...
				ip, netmask, gateway,entry.metric,(entry._interface_?"WAN":"LAN"), i);
	}
	return nBytesSent;
}
...
int CheckRouteType(STATICROUTE_Tp checkentry)
{
	...
	for (i=1; i<=entryNum; i++) {
		...
		*((char *)&entry) = (char)i;
		...
		if (dhcp == PPPOE || dhcp == PPTP || dhcp == L2TP) {
			if(entry._interface_ ==1){
				if(!memcmp(checkentry->dstAddr, &entry.dstAddr, 4) && !memcmp(checkentry->netmask, &entry.netmask, 4) && entry._interface_ == checkentry->_interface_ ){
					isFound=0;		
					break;
				}
			}else{
				if(!memcmp(checkentry->dstAddr, &entry.dstAddr, 4) && !memcmp(checkentry->netmask, &entry.netmask, 4) && !memcmp(checkentry->gateway, &entry.gateway, 4) && entry._interface_ == checkentry->_interface_){
					isFound=0;		
					break;
				}
			}
...
int kernelRouteList(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	nBytesSent += websWrite(wp, T("<tr>"
			...
			sdest, sgw, inet_ntoa(mask), metric, (entry._interface_?"WAN":"LAN"),(entry_type?"Static":"Dynamic"));
			}
		}
		nl++;
	}
