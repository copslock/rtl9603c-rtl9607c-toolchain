...
#define IP_NAT_PORT_SHIFT_MAPPING 0x80 //it's a special RTK's flag .(also define in ip_nat_core.c)
			star = strchr(colon, '*');
			if(!star)								
				range.shift = 0;
			else
			{				
				range.shift = atoi(star+1);
				range.flags |= IP_NAT_PORT_SHIFT_MAPPING; //make this flag can only be realized in rtk's port DNAT
			}		
#endif			
		}
