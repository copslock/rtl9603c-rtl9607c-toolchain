...
//------------------------------------------------
  ...
  //if l2tp server is not in the same subnet as l2tp client, we put l2tp server ip to /var/l2tp_server
  	fp1= fopen("/var/sys_op", "r");
	if (fp1!=NULL) {
        	fscanf(fp1,"%d",&system_op);
		fclose(fp1);
	...
	memset(server_ip_in_IPAddr, '\0', 30);
    	memcpy(&retval.s_addr, hp->h_addr, sizeof(retval.s_addr));
	strcpy(server_ip_in_IPAddr,inet_ntoa(retval));
	if (getInAddr(curr_iface, IP_ADDR, (void *)&intaddr )){
  	if(getInAddr(curr_iface, SUBNET_MASK, (void *)&intmask)){
  		if((intaddr.s_addr & intmask.s_addr) != (retval.s_addr & intmask.s_addr)){
  			//warn("generate pptp server =%s\n", inet_ntoa(retval));
  			sprintf(server_ip, "echo %s > /var/l2tp_server", server_ip_in_IPAddr);
  			system(server_ip);
				...
				if(fp1 !=NULL){
					...
					if(l2tp_ipdyn[0]=='1'){
						...
						if (fp2 != NULL) {
							...
							sprintf(cmdBuffer, "route add -host %s gw %s",server_ip_in_IPAddr, l2tp_gw);
							system(cmdBuffer);
        						fclose(fp2);
   						}
					}else{
				 ...
				 //if l2tp server is in the same subnet as l2tp client, we add the route entry for single host				
			 	sprintf(cmdBuffer, "route add -host %s gw %s",server_ip_in_IPAddr, server_ip_in_IPAddr);				
			 	system(cmdBuffer);
...
void lac_Inform_PPPD(int cid)
{
    ...
    while (t)
    {
        ...
        while (tmp)
        {
            if (tmp->ourcid == cid)
            {
                log (LOG_LOG,"%s : Inform call pppd to mconnect pid=%d\n",__FUNCTION__,tmp->pppd);
			if(tmp->pppd > 0){
	                kill(tmp->pppd, SIGUSR2);
	                return;
	            }
            }
            tmp = tmp->next;
    ...
    log (LOG_DEBUG, "%s : No such call %d to Inform.\n", __FUNCTION__, cid);
    return;
}
...
/*				tmp->needclose = -1; */
                ...
                system("echo kill > /var/pppdkilled"); 
                 //log (LOG_DEBUG, "%s: call disconnect.sh option!!\n", __FUNCTION__);	
    ...
    while (cnt)
    {
        ...
        if (cnt > 0)
        {
            ...
            case 's':
                show_status ();
                break;
		case 'f': /*force pppd to connect in call on demand mode*/
			tunstr = strchr (buf, ' ') + 1;
                	lac = laclist;
                	while (lac)
                	{
                ...
                if(found_session==1 && gconfig.connect_mode==2){
			lac_Inform_PPPD(call);	
                }
                break;		
            default:
...
void init_args(int argc, char *argv[]) {
    ...
    gconfig.daemon=1;
    if ( stat(DEFAULT_PID_FILE, &pidfile_status) == 0){
    		unlink(DEFAULT_PID_FILE);//remove the previous pid file, let l2tpd to create again
    }
    ...
    for (i = 1; i < argc; i++) {
        if(! strncmp(argv[i],"-c",2)) {
            ...
            if(++i == argc)
                ...
                gconfig.connect_mode=atoi(argv[i]);
        }
        else {
            usage();
        }
