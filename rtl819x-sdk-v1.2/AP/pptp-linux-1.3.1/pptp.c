 ...
 *
 * $Id: pptp.c,v 1.12 2010/03/10 05:30:09 keith_huang Exp $
 */
...
#if defined(CONFIG_RTL8186_TR) || defined(CONFIG_RTL865X_SC) || defined(CONFIG_RTL865X_AC) || defined(CONFIG_RTL865X_KLD) || defined(CONFIG_RTL8196C_EC)
  //Brad add   
  //if pptp server is not in the same subnet as pptp client, we put pptp server ip to /var/pptp_server
	if (getInAddr(curr_iface, IP_ADDR, (void *)&intaddr )){
	  	if(getInAddr(curr_iface, SUBNET_MASK, (void *)&intmask)){
	  		if((intaddr.s_addr & intmask.s_addr) != (retval.s_addr & intmask.s_addr)){
	  			sprintf(server_ip, "echo %s > /var/pptp_server", inet_ntoa(retval));
	  			system(server_ip);
	  			
				fp1= fopen("/var/pptp_dyn", "r");
				if(fp1 !=NULL){
					fscanf(fp1, "%s", pptp_ipdyn);
					if(pptp_ipdyn[0]=='1'){
						fp2= fopen("/var/pptp_gw", "r");
						if (fp2 != NULL) {
							fscanf(fp2, "%s", pptp_gw);
							sprintf(cmdBuffer, "route add -host %s gw %s", inet_ntoa(retval), pptp_gw);
							system(cmdBuffer);
	        					fclose(fp2);
	   					}
					}else{
				 ...
				 //if pptp server is in the same subnet as pptp client, we add the route entry for single host
				sprintf(cmdBuffer, "route add -host %s gw %s", inet_ntoa(retval), inet_ntoa(retval));
				system(cmdBuffer);
			}
		}
		
		sprintf(cmdBuffer, "echo \"%08X %08X\" > /proc/pptp_src_ip", intaddr.s_addr , retval.s_addr);
		system(cmdBuffer);
	}
#else
