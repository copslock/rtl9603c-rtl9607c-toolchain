...
static int logFileSize = 32 * 1024;
...
#if 1 // kenny: /* Always send console messages to /dev/console so people will see them.=> deafult not enable */ 
	if ((fd =
...
// david ---------------------------------------------------
	{
		unsigned int sec, mn, hr, day;
		
		time(&now);

		day = now / 86400;
		day -= 10957; // day counted from 1970-2000
		if (day > 4*365) // date > 2004, use system time
			goto use_sys_time;
		...
		sprintf(timestamp, "%dday %02d:%02d:%02d", day, hr, mn, sec);			
use_sys_time:
//----------------------------------------------------------
