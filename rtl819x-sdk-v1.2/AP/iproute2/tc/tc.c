...
/*plus add:declare extern Queue Disc*/
...
//extern struct qdisc_util hpfg_util;
...
//	void *dlh;
	...
	//q = dlsym(dlh, buf);
#if 1	
	if(!strcmp("cbq_util",buf))
		q=&cbq_util;
	else if(!strcmp("sfq_util",buf))
		q=&sfq_util;
	else if(!strcmp("red_util",buf))
		q=&red_util;
	else if(!strcmp("prio_util",str))
		q=&prio_util;
	else if(!strcmp("ingress_util",buf))
		q=&ingress_util;
	else if(!strcmp("htb_util",buf))
		q=&htb_util;
//	else if(!strcmp("hpfg_util",buf))
//		q=&hpfg_util;
//	else if(!strcmp("hfsc_util",buf))
//		q=&hfsc_util;
	else if(!strcmp("gred_util",buf))
		q=&gred_util;
	else if(!strcmp("pfifo_util",buf))
		q=&pfifo_util;
	else if(!strcmp("gred_util",buf))
		q=&gred_util;
//	else if(!strcmp("csz_util",buf))
//		q=&csz_util;
	else
...
#endif
	snprintf(buf, sizeof(buf), "%s_util", str);
	//q = dlsym(dlh, buf);  //plus modify :mark it for we change link way (shared lib-> static lib)

#if 1
	if(!strcmp("u32_util",buf))
		q = &u32_util	;
	else if(!strcmp("tcindex_util",buf))
		q = &tcindex_util	;
	else if(!strcmp("rsvp6_util",buf))
		q = &rsvp6_util	;
	else if(!strcmp("route_util",buf))
		q = &route_util	;
	else if(!strcmp("fw_util",buf))
		q = &fw_util	;
	else 
		printf("p:Unknown filter\n");
#endif
